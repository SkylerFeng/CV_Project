...
def upsample_bicubic(lr: Image.Image, size_wh):
    """LR -> upsampled to HR size using bicubic"""
    return lr.resize(size_wh, resample=Image.BICUBIC)


def upsample_lanczos(lr: Image.Image, size_wh):
    """LR -> upsampled to HR size using Lanczos"""
    return lr.resize(size_wh, resample=Image.LANCZOS)
...
def make_vis_grid(
    ...
    sr: Image.Image,
    hr: Image.Image,
    bicubic: Image.Image = None,
    lanczos: Image.Image = None
):
    """Return a single PIL image concatenating LR_up / Bicubic / Lanczos / SR / HR"""
    if bicubic is None:
        bicubic = lr_up
    if lanczos is None:
        lanczos = lr_up

    images = [lr_up, bicubic, lanczos, sr, hr]
    w, h = hr.size
    canvas = Image.new("RGB", (w * 5, h))
